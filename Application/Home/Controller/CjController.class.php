...
class CjController extends Controller {
    public function index() {
		...
		$publishTime = $this->getNeedBetween2($data22,'publishTime:',',',12);
		...
		$url = 'http://s1-cdn.eqxiu.com/eqs/page/' . $arr22['id'].'?code='.$codee.'&time='.$publishTime;
        ...
        if (empty($code) and $arr22[name] !== '该场景已关闭') {
			...
			foreach ($filepath as $key => $var) {
				...
				if(preg_match('|^http://|', $var)){
					...
					$this->save_pic('http://res1.eqh5.com/' . $var, $img);
				}  
			}
