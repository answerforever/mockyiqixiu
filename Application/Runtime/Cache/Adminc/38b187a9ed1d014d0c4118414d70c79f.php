	...
	margin-left: 0 !important;}</style>
				...
				<div class="row-fluid">
					...
					<div class="span12">
						...
						<h3 class="page-title">
							 ...
							 <?php if(($user[userid_int]) == "0"): ?>系统模板编辑模板<small>   Template editing system</small><?php else: ?>用户案例编辑 <small>   Edit user case</small><?php endif; ?> 
						...
						<ul class="breadcrumb">
							...
							<li><a href="#"> <?php if(($user[userid_int]) == "0"): ?>系统模板编辑<?php else: ?>用户案例编辑<?php endif; ?> </a></li>
		  ...
		  <div class="tabbable tabbable-custom tabbable-full-width">
							<ul class="nav nav-tabs">
                               <li class=""> 
<?php if(($user[is_tpl]) == "1"): ?><a href="<?php echo U('scene/index');?>"> 系统模板列表</a> <?php else: ?> <a href="<?php echo U('/Scene/index/flag/useranli');?>"> 用户案例列表</a><?php endif; ?> 
            </li>
                <li class="<?php if(($user[userid_int]) == "0"): ?>active<?php else: ?>active<?php endif; ?> "> 
                <a href=""> <?php if(($user[userid_int]) == "0"): ?>系统模板编辑<?php else: ?>用户案例编辑<?php endif; ?> </a> </li>
							</ul>
                            <hr>
                </div>
   ...
   <input name="ctl00$ContentPlaceHolder1$txt" type="hidden" id="ctl00_ContentPlaceHolder1_txt" />
                ...
                <form action="" class="form-horizontal" method="post">
							...
							<img src="/Uploads/<?php echo ((isset($user["thumbnail_varchar"]) && ($user["thumbnail_varchar"] !== ""))?($user["thumbnail_varchar"]):"defaultuser.jpg"); ?>" width="180" height="180" />
                            <img src="http://qr.liantu.com/api.php?w=180&m=10&text=<?php echo ($_SERVER['HTTP_HOST']); ?>/v-<?php echo ($user["scenecode_varchar"]); ?>" style="border: 1px solid #CCC;"/>
										</div>
							...
							<div class="portlet-body form">
								...
								<!-- BEGIN FORM-->
									...
									<div class="control-group">
										<label class="control-label"><?php if(($user[userid_int]) == "0"): ?>模板名<?php else: ?> 案例名称<?php endif; ?></label>
										<div class="controls">
<input name="user[scenename_varchar]" type="text"   class="m-wrapt" value="<?php echo ($user["scenename_varchar"]); ?>" style="width:360px"/>
                              ...
                              <div class="control-group">
													...
													<div class="controls">
<select name="user[movietype_int]" style="width:180px">
<option value="">选择</option>
  <?php if(is_array($movietypelist)): foreach($movietypelist as $key=>$item): ?><option value="<?php echo ($key); ?>" <?php if(($key) == $user['movietype_int']): ?>selected<?php endif; ?>><?php echo ($item); ?></option><?php endforeach; endif; ?>   
 ...
 <select name="user[scenetype_int]" id="scenetypeB" onChange="changeSceneType()" style="width:120px" >
                       ...
                       <select name="user[tagid_int]" id="scenetypeS" style="width:120px" >
...
<?php if(($user[is_tpl]) == "1"): ?><div class="control-group">
										<label class="control-label">消耗秀点</label>
										<div class="controls">
											 
                                            
                                   <input name="user[price]" type="text"   class="m-wrapt" style="width: 200px;" value="<?php echo ($user["price"]); ?>" /> 
 ...
 <?php else: ?>
	  <?php if(C('SYS_LINK')): ?>
           <div class="control-group">
                                   ...
                                   <input name="user[vi_hittimes_set]" type="text"   class="m-wrapt" style="width: 200px;" value="<?php echo ($user["vi_hittimes_set"]); ?>" /> 
     			 ...
     			 <div class="control-group">
                                    ...
                                    是否开启 <input type="checkbox" value="1" name="user[vi_timeout][enable]" <?php if($user[vi_timeout][enable]==1): ?>checked="checked" <?php endif; ?>>  <br /> 
                             限制场景打开的时间<input name="user[vi_timeout][time]" type="text"    style="width: 100px;" value="<?php echo ($user["vi_timeout"]["time"]); ?>" /> （单位分钟）
			...
			<div class="controls">
<label class="radio"> <input id="ctl00_ContentPlaceHolder1_rdiIsEnable_help_1" type="radio"   name="user[delete_int]" value="0" <?php if(($user['delete_int']) == "0"): ?>checked="checked"<?php endif; ?>/>启用</label>

<label class="radio"> <input id="ctl00_ContentPlaceHolder1_rdiIsEnable_help_0" type="radio" name="user[delete_int]" value="1" <?php if(($user['delete_int']) == "1"): ?>checked="checked"<?php endif; ?>/>禁用</label>  
...
<div class="footer">
		...
		<div class="footer-inner">
			...
			搜虎精品社区www.souho.net提供源码，持续更新升级中
